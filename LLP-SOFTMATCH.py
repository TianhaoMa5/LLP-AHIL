...
def set_model(args):
    ...
    if args.eval_ema:
        ...
        ema_model.fc = nn.Linear(model.fc.in_features, args.n_classes)
...
@torch.no_grad()
def update_prob_t(args,ema_probs,mean,var, ulb_probs):
    ulb_prob_t = ulb_probs.mean(0)
    ema_probs = args.ema_p * ema_probs + (1 - args.ema_p) * ulb_prob_t

    max_probs, max_idx = ulb_probs.max(dim=-1)
    prob_max_mu_t = torch.mean(max_probs)
    prob_max_var_t = torch.var(max_probs, unbiased=True)
    mean = args.ema_p * mean + (1 - args.ema_p) * prob_max_mu_t.item()
    var = args.ema_p * var + (1 - args.ema_p) * prob_max_var_t.item()
    return ema_probs,mean,var

@torch.no_grad()
def calculate_mask(probs,mean,var):
...
def train_one_epoch(epoch,
                    ...
                    logger,
                    samp_ran,
                    ema_maxprob,ulb_prob_t,prob_max_mu_t,prob_max_var_t
                    ):
    model.train()
    ...
    for it in range(len(dltrain_u)):
        ...
        with torch.no_grad():
            ...
            ulb_prob_t,prob_max_mu_t,prob_max_var_t=update_prob_t(args, ulb_prob_t,prob_max_mu_t,prob_max_var_t, probs)
            max_probs, mask=calculate_mask(probs, prob_max_mu_t, prob_max_var_t)
    ...
    return loss_u_meter.avg, loss_prop_meter.avg, n_correct_u_lbs_meter.avg, samp_lb_meter, samp_p_meter,ema_maxprob,entropy_meter.avg,ulb_prob_t,prob_max_mu_t,prob_max_var_t
...
def main():
    ...
    parser.add_argument('--exp-dir', default='L^2P-SOFT', type=str, help='experiment id')
    ...
    ulb_prob_t = torch.ones((args.n_classes)).cuda() / args.n_classes
    prob_max_mu_t = 1.0 / args.n_classes
    prob_max_var_t = 1.0

    for epoch in range(args.n_epoches):
        loss_u, loss_prob, n_correct_u_lbs, samp_lb, samp_p,ema_maxprob,entropy_meter,ulb_prob_t,prob_max_mu_t,prob_max_var_t = \
            train_one_epoch(epoch, bagsize=args.bagsize, n_classes=args.n_classes, **train_args,samp_ran=samp_ran,ema_maxprob=ema_maxprob,ulb_prob_t=ulb_prob_t,prob_max_mu_t=prob_max_mu_t,prob_max_var_t=prob_max_var_t)
